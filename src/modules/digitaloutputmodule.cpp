...
DigitalOutputModule::DigitalOutputModule()
    : _outputNumber(-1), _statusTopic(""), _getTopic(""), _setTopic("") {}
...
void DigitalOutputModule::deserialize(const JsonObject& configJson) {
  DynamicJsonBuffer jsonBuffer;
...
void DigitalOutputModule::mqtt(unsigned int type,
                               const char* topic,
                               const char* payload) {
...
bool DigitalOutputModule::_isValid() {
  return _outputNumber > 0 && _statusTopic.length() > 0 &&
         _getTopic.length() > 0 && _setTopic.length() > 0;
}
